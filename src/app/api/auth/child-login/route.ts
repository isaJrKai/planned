...
const MASCOT_MAP: Record<string, string> = { "turtle":"turtle","rabbit":"rabbit","chick":"chick","panda":"panda","penguin":"penguin","koala":"koala","fox":"fox","dolphin":"dolphin","owl":"owl","squirrel":"squirrel","raccoon":"raccoon","eagle":"eagle","wolf":"wolf","lion":"lion","octopus":"octopus","dragon":"dragon","leopard":"leopard" };
export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { mascot, name, pin } = body;
    if (!mascot || !name || !pin || pin.length !== 4) return NextResponse.json({ ok: false, error: "Pick your mascot, enter your name and secret code." }, { status: 400 });
