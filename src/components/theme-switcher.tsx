...
// ============================================================================
// THEME SWITCHER — Planned Kids Savings
// ============================================================================
// 4 swatches: dark / light / pink / red. Persists to localStorage and
// applies data-theme attribute to <html>.
// ============================================================================

import { useEffect, useState } from "react";

export type ThemeId = "dark" | "light" | "pink" | "red";

const THEMES: { id: ThemeId; label: string; swatch: string[] }[] = [
  {
    id: "dark",
    label: "Onyx",
    swatch: ["#090C0A", "#1A332A", "#C9A84C"],
  },
  {
    id: "light",
    label: "Ivory",
    swatch: ["#FAF7EF", "#FFFFFF", "#B8941F"],
  },
  {
    id: "pink",
    label: "Blush",
    swatch: ["#FDF4F1", "#D4869A", "#C9A84C"],
  },
  {
    id: "red",
    label: "Oxblood",
    swatch: ["#1F0606", "#8B2635", "#D4AF37"],
  },
];
...
export function useTheme() {
  ...
  // Apply to <html> whenever it changes (and persist).
  useEffect(() => {
      ...
      >
        <div className="flex -space-x-1">
          {THEMES.find((t) => t.id === theme)?.swatch.map((c, i) => (
            <span
              key={i}
              className="h-3 w-3 rounded-full border border-[rgba(201,168,76,0.4)]"
              style={{ background: c }}
            />
          ))}
        </div>
          ...
          <div className="absolute right-0 top-full mt-2 z-40 surface-wood-strong rounded-lg p-3 min-w-[180px] shadow-2xl">
            ...
            {THEMES.map((t) => (
              ...
              >
                ...
                <span
                  className="text-xs tracking-wider flex-1"
                  style={{
                    color: theme === t.id ? "var(--chart-5)" : "var(--text-soft)",
                  }}
                >
                  {t.label}
                </span>
